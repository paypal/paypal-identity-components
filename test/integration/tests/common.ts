...
export function buildSDKScriptUrl(query: Record<string, any> = {}): string {
...
export function runOnClick<T>(handler: () => T): T | undefined {
...
export function onWindowOpen({
  ...
  return new ZalgoPromise<Window>((resolve, reject) => {
...
export const assert = {
  ok(item: unknown, message?: string) {
    if (!item) {
      throw new Error(
  ...
  equal(one: unknown, two: unknown, message?: string) {
    if (one !== two) {
      throw new Error(
  ...
  notEqual(one: unknown, two: unknown, message?: string) {
    if (one === two) {
      throw new Error(
        message ||
