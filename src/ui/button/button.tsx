...
import type { ButtonStyle, ButtonPropsInputs } from "./props";
import { normalizeButtonProps } from "./props";
...
function PPSymbol({ style }: LogoProps): ComponentNode {
...
function Label({
  style,
  content,
}: LabelProps): ComponentNode | null | undefined {
  const { label } = style;
  const text = content;
...
export function AuthButton(props: ButtonProps): ElementNode {
  ...
  const clickHandler = (event: MouseEvent | KeyboardEvent, opts: EventInit) => {
    event.preventDefault();
    event.stopPropagation();
    // @ts-expect-error event target is possibly null
    event.target.blur();
    onClick(event, {
      fundingSource,
      ...opts,
  ...
  const keypressHandler = (event: KeyboardEvent, opts: KeyboardEventInit) => {
