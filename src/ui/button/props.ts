import type { $Values, $PropertyType } from "utility-types";
...
import { CrossDomainWindowType } from "@krakenjs/cross-domain-utils/dist/esm";
...
export type OnClick = (arg0: OnClickData, arg1: OnClickActions) => void;
...
export type RenderButtonProps = {
  ...
  locale: LocaleType;
  fundingSource: $Values<typeof FUNDING> | null | undefined;
...
export type PrerenderDetails = {
  ...
  card: $Values<typeof CARD> | null | undefined;
};
...
export function normalizeButtonProps(
  props: ButtonPropsInputs | null | undefined
): RenderButtonProps {
  if (!props) {
  ...
  return {
    clientID,
    fundingSource,
    // @ts-expect-error TS is confused because style is changing types after normalization
    // probably better practice to use a new variable for normalizedStyle?
    style,
    locale,
    env,
    platform,
